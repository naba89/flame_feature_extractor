...
class ImageEngine:
    ...
    def __call__(self, image_tensor, image_key="default"):
        ...
        return {
            "image": image_tensor.cpu().float() / 255.0,
            "image_key": image_key,
            "warped_image": warped_image.cpu().float(),
...
def crop_and_resize(image_tensor, bbox_xyxy, t_size=224, t_scale=1.25):
    ...
    warped_image = torchvision.transforms.functional.crop(image_tensor, target_top, target_left, target_hw, target_hw)
    warped_image = torchvision.transforms.functional.resize(warped_image, size=(t_size, t_size), antialias=True)
    return warped_image
