...
class EmicaCore:
            ...
            if if_crop:
                ...
                for fidx, frame in tqdm(enumerate(frames_data), total=frames_data.shape[0]):
                    if meta_data["video_fps"] > 50:
                        if fidx % 2 == 0:
                            continue
                    frame = tf.resize(frame, 512, antialias=True)
                    frame = tf.center_crop(frame, 512)
                    if matting:
                        frame = (
                            self.matting_engine(frame / 255.0, return_type="matting", background_rgb=background).cpu()
                            * 255.0
                        )
    ...
    def crop_image(self, inp_image):
        ...
        crop_image = tf.crop(inp_image, top=bbox[1], left=bbox[0], height=bbox[3] - bbox[1], width=bbox[2] - bbox[0])
        crop_image = tf.resize(crop_image, (512, 512), antialias=True)
        return crop_image
